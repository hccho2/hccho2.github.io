...
* squashing function $$\tanh$$: 보통 continuous action space에서 policy network은 정규분포로 모델링하고 action range를 벗어나는 action은 clipping으로 처리한다. SAC논문에서는 $$u\sim N(\mu,\sigma)$$를 예측하고 $$a=\tanh(u)$$로 action을 예측했다. 따라서 action $$a$$의 likelihood(pdf값)은 change of variables를 이용해서 계산해주면 된다. 
