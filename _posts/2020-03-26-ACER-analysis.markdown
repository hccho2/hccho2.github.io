...
* Policy Loss: $$\widehat{g}_t^{acer}$$의 앞부분: $$V^\pi(x_t) =  \sum_{a_t} \pi(a_t|x_t) Q^\pi(x_t,a_t)$$와 $$ \overbrace{Q^{\mbox{\small ret}}(x_t, a_t)}^{a_t\text{는 trajectory action}}$$로 부터 
advantage $$A_t: =\underbrace{Q^{\mbox{\small ret}}(x_t, a_t)}_{\text{batch-size}} - \underbrace{V^\pi(x_t)}_{\text{batch-size}}$$를 계산할 수 있고, 이로 부터 Policy Loss를 다음과 같이 구할 수 있다.
