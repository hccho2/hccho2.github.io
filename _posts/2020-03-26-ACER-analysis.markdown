...
그래서 new, old policy간 기대값 변환에 필요한 important sampling weight $$\rho$$가 필요하다. 또한 train이 되면서 계속 변하는 new policy가 data를 만들어낸 old policy로 부터 많이 도망가지 못하게 제약을 주거나 trust region을 설정한다.
