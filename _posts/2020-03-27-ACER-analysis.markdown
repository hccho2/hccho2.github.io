...
$$
\begin{eqnarray*}
g_t^{marg}  &=& \mathbb{E}_{a_t\sim\mu}\Big[ \bar{\rho}_{t}  \nabla_{\theta} \log \pi_{\theta}(a_t \vert x_t) Q^\pi(x_t, a_t)  \Big]
...
* old policy에 대한 기대값이므로, off policy 환경에서 계산을 위하여 $$Q^\pi$$를 $$Q^{ret}$$로 대체하여 $$\widehat{g}_t^{marg}$$을 다음과 같이 정의한다.
